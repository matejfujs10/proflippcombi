  ...
  return (
    <>
      <section id="kontakt" className="relative py-24 md:py-36 overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${combiSunset})` }} />
        <div className="absolute inset-0 bg-gradient-to-b from-background/95 via-background/80 to-background" />

        <div className="relative container mx-auto px-4 text-center max-w-3xl">
          <span className="section-eyebrow mb-4">{t("finalCta.eyebrow", lang)}</span>
          <h2 className="font-display text-5xl md:text-7xl lg:text-8xl text-foreground mt-4 mb-6 leading-none">
            {t("finalCta.h1", lang)} <span className="text-gradient">{t("finalCta.h2", lang)}</span>
          </h2>

          <div className="space-y-1 text-lg md:text-2xl text-foreground/85 font-heading font-semibold mb-3">
            <p>{t("finalCta.l1", lang)}</p>
            <p>{t("finalCta.l2", lang)}</p>
            <p className="text-gradient">{t("finalCta.l3", lang)}</p>
          </div>
          <p className="text-base md:text-lg text-muted-foreground mb-10">{t("finalCta.sub", lang)}</p>

          <div className="flex flex-col sm:flex-row gap-3 justify-center mb-10">
            <button onClick={() => setOpen(true)} className="btn-hero">
              <Send size={18} className="mr-2" /> {t("finalCta.book", lang)}
            </button>
            <a href="mailto:info@proflipp.com" className="btn-ghost-light">{t("finalCta.inquiry", lang)}</a>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6 text-foreground/80">
            ...
            <span className="hidden md:inline text-border">|</span>
            <a href="https://wa.me/38668169430" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-accent transition">
